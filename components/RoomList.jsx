   ...
   const handleUploadPhoto = async (roomId) => {
     ...
     try {
       ...
       const response = await axios.post(
         `http://localhost:4001/api/rooms/${roomId}/photos`,
         { imageUrl }
       );
       ...
       const updatedRooms = rooms.map((room) => {
         if (room._id === roomId) {
           const updatedPhotos = [...room.photos, imageUrl];
       ...
       console.error(error);
       // Handle photo upload errors here (display error message, etc.)
     }
   };
...
<div>
  ...
  <img src={photo} alt={`Room photo ${index}`} style={{ marginRight: "10px" }} />
  <button onClick={() => handleDeletePhoto(room._id, index)}>S</button>
</div>
        ...
        <div>
          ...
          <button onClick={() => handleUploadPhoto(room._id)}>
            Ajouter une image
          </button>
        </div>
        <Link

...
export const createRoom = async (req, res, next) => {
  ...
  const imageUrls = req.body.photos; // Assuming the image URLs are sent in the request body

  try {
    newRoom.photos = imageUrls; // Set the photos field to the array of image URLs
...
export const updateRoom = async (req, res, next) => {
  ...
  try {
    const updatedRoom = await Room.findByIdAndUpdate(
      id,
      { ...updates, photos: imageUrls }, // Set the photos field to the array of image URLs
      { new: true }
    );
